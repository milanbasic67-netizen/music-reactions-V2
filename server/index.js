...
// --- RUTA: IMPORT YOUTUBE (VERZIJA 52 - PRO & HEAVY BYPASS) ---
app.post("/import-youtube", async (req, res) => {
    const { url } = req.body;
    const RAPID_KEY = '01f396de62msh53c99a3cb08ea27p1908ecjsnc9856c6b2fea';
    const videoId = getYTID(url);

    console.log("\n--- YOUTUBE IMPORT (V52 - PRO MODE) ---");

    try {
        // 1. Dobijanje linka - tražimo najniži kvalitet
        const apiRes = await axios.get('https://social-media-video-downloader.p.rapidapi.com/youtube/v3/video/details', {
            params: { videoId, urlAccess: 'proxied', renderableFormats: '240p' },
            headers: { 'x-rapidapi-key': RAPID_KEY, 'x-rapidapi-host': 'social-media-video-downloader.p.rapidapi.com' }
        });

        // Uzimamo prvi video link koji nam API ponudi
        const mp4Url = apiRes.data?.contents?.[0]?.videos?.[0]?.url;
        if (!mp4Url) throw new Error("Video link nije pronađen.");

        console.log("Pokrećem direktan cloud-to-cloud prenos...");

        // 2. Dobijamo stream sa YouTube-a
        ...
        const size = ytStream.headers['content-length'];
        
        console.log(`Veličina fajla koja se šalje: ${size ? (size/1024/1024).toFixed(2) + " MB" : "Nepoznato"}`);

        // 3. Šaljemo na Supabase koristeći Axios (pouzdanije za velike fajlove)
        const supabaseUrl = `${process.env.SUPABASE_URL}/storage/v1/object/songs/${videoName}`;
        
        const upRes = await axios.post(supabaseUrl, ytStream.data, {
            headers: {
                'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`,
                'API-Key': process.env.SUPABASE_SERVICE_ROLE_KEY,
                'Content-Type': 'video/mp4',
                'x-upsert': 'true',
                // Ako imamo veličinu, šaljemo je da Supabase Pro ne bi blokirao
        ...
        console.log("Uspeh! Video je na Supabase-u.");
        ...
        res.json({ 
            ...
            artist: apiRes.data.metadata?.author?.name || "Unknown"
        });

    } catch (err) {
        console.error("DETALJI GREŠKE:", err.response?.data || err.message);
        res.status(500).json({ 
            error: "Import failed", 
...
app.listen(PORT, () => console.log(`Backend V52 Online`));