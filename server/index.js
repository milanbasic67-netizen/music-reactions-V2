...
app.post("/import-youtube", async (req, res) => {
    ...
    console.log("\n--- YOUTUBE IMPORT (FINAL STABLE PIPE) ---");
            ...
            supRes.on("end", () => {
                if (supRes.statusCode >= 200 && supRes.statusCode < 300) {
                    ...
                    res.json({ 
                        ...
                        artist: apiRes.data.metadata?.author?.name || "Unknown Artist"
                    });
                } else {
                    res.status(500).json({ error: "Supabase upload failed", details: resData });
                }
            });
        });

        https.get(mp4Url, (ytRes) => {
            if (ytRes.statusCode === 200) ytRes.pipe(supabaseRequest);
            else supabaseRequest.destroy();
        });
...
app.listen(PORT, () => console.log(`Server Online - V44`));