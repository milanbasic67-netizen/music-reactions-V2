...
// POMOĆNA FUNKCIJA: Izvlači samo ID videa (sprečava 404 greške)
...
// 1. RUTA: IMPORT YOUTUBE (Fiksirana putanja)
app.post("/import-youtube", async (req, res) => {
    const { url } = req.body;
    const videoId = getYTID(url);
    
    console.log("\n--- YOUTUBE IMPORT ZAPOČET --- ID:", videoId);

    if (!videoId) return res.status(400).json({ error: "Invalid YouTube URL" });

    try {
        // KORISTIMO STABILNIJI ENDPOINT
        const options = {
            method: 'GET',
        ...
        // API obično vraća listu linkova u 'link' ili 'urls' polju
        // Uzimamo prvi MP4 link koji nađemo
        const mp4Url = data.link || data.urls?.[0]?.url || data.url;
        ...
        res.status(500).json({ error: "YouTube Import Failed. Check API subscription." });
    }
});
...
app.get("/", (req, res) => res.send("TikTok Duet Server Online (V3)"));
